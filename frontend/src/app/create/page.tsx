...
export default function CreateBlockchainPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(data.message || 'Failed to create blockchain')
      }
    } catch (error) {
      console.error('Blockchain creation error:', error)
      alert('Failed to create blockchain. Please try again.')
    } finally {
      setIsCreating(false)
    }
  }
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await fetch(`/api/blockchain/${createdBlockchain.id}/export`)
      ...
      if (response.ok) {
        ...
        throw new Error('Failed to download blockchain')
      }
    } catch (error) {
